...
2.x 系列使用 Flutter 对桌面端进行了全面重写，不再依赖 JVM 运行时，UI 与窗口交互更贴近原生系统体验。相比 1.x（Kotlin Multiplatform / Compose Desktop）版本，本版本在启动速度、播放性能、平台覆盖和功能完整性上均有质的飞跃。

- **Flutter 原生重写**：2.x 系列基于 Flutter 框架重新构建桌面端，渲染与窗口调度更贴近原生系统，告别 JVM 运行时的额外开销，安装包更轻量、运行更高效。
- **启动与起播更快**：启动链路、播放器初始化与首帧渲染均经过优化，应用打开与视频起播速度较 1.x 显著提升。
- **内置全平台硬解播放器，本地解码能力大幅增强**：采用 fvp（基于 libmdk / FFmpeg）作为播放内核，支持 GPU 硬解与本地解码，可直接处理 HDR / HLG / Dolby Vision 等动态范围视频及 PGS/SUP 位图字幕。相比 1.x 受限于 Compose Desktop 渲染机制、硬解画面需经 RGBA 回读 CPU 再渲染导致的高 CPU 占用与被迫服务端转 SDR，2.x 显著降低 CPU 占用并保留原片动态范围。
...
- **支持更多平台与 CPU 架构**：1.x 仅覆盖 Windows 与 macOS；2.x 新增 Linux，完整覆盖 Windows x64、macOS x64 / arm64（Intel / Apple Silicon）、Linux x64 / arm64，安装包格式包括 .exe、.dmg、.deb、.rpm、.pkg.tar.zst 与 .AppImage。
- **功能更完整**：新增文件夹视图、网盘视频播放、 Live TV、媒体信息面板、播放详细信息面板，以及强制 H.264 / SDR 色调映射等进阶播放选项。
...
播放器基于 `fvp`（libmdk / FFmpeg），支持 GPU 加速能力，但最终效果取决于平台、驱动、视频格式以及系统环境。HDR、字幕、音轨切换等体验仍在持续优化，具体请以实际版本表现为准。
...
- [fvp](https://github.com/wang-bin/fvp) - 基于 libmdk 的 Flutter 音视频播放插件
