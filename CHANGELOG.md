...
- **自签名证书支持**：连接服务器遇到证书校验失败（如自签名、过期证书）时，可选择信任该证书并继续使用；已信任的证书可在设置中管理。
- **继续观看卡片**：首页继续观看卡片新增「更多」菜单，优化海报封面布局。
- **直连播放字幕说明**：云盘直连播放转码画质时，字幕面板与飞牛影视 Web 版一致，对直连流无法提供的内置字幕给出清晰提示。
- **快捷键设置**：快捷键设置弹窗新增取消按钮。

### Changed

- **海报卡片**：影片与剧集海报的悬停遮罩样式与飞牛影视 Web 版对齐，并修复剧集「更多」按钮菜单无法弹出的问题。
- **云盘会员徽标**：播放时的云盘会员徽标图标与飞牛影视 Web 版保持一致。
- **设置页**：隐私声明移入「隐私与安全」分组。

### Fixed

- **Windows 登录**：修复了 Windows 正式安装包中登录页无法加载、输入框不可见的问题。
- **云盘播放**：修复了云盘播放正常时仍误报播放错误的问题。
- **全屏播放**：修复了全屏模式下画面未完全填充横向空间、重新进入播放器时全屏形态未恢复的问题。
- **快进快退**：修复了使用快捷键快退后播放位置不准确的问题。
- **详情页加载**：修复了详情页加载失败时显示不可读错误信息的问题。
- **字幕推荐**：修复了非剧集内容也显示相似字幕推荐的问题。
- **播放详情面板**：修复了打开播放器中的播放详情面板动画卡住的问题。
...
- **片头片尾快捷设置**：设置片头片尾时，可一键把当前播放位置设为片头结束点或片尾开始点，无需手动输入时间。
- **播放详情面板**：播放详情面板改用液态玻璃材质，并添加灵动的过渡动画；面板关闭按钮固定显示，滚动查看内容时无需再向上翻找。
- **媒体信息空状态**：文件媒体信息中无视频、音频或字幕轨道时，显示与其他飞牛影视 Web 版一致的提示插图。
- **演职人员**：演职人员职务改为中文显示；人物信息缺失时显示提示，而不是报错。

### Changed

- **提示框视觉效果**：优化了提示框的玻璃质感与边缘反光效果。
- **macOS 窗口外观**：优化 macOS 窗口顶部的白色高光线条。
...
- **Linux 安装包**：修复了 Linux 安装包在 Arch Linux 上无法启动的问题。
