...
class StreamSelector<T> extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final selectedItem = items.where((i) => i.value == selectedValue).firstOrNull;
    final label = selectedItem?.label ?? placeholder ?? 'Select';
      ...
      items: items.map((item) {
        return MenuFlyoutItem(
          ...
          trailing: item.value == selectedValue
              ? Icon(FluentIcons.check_mark, size: 12, color: FluentTheme.of(context).accentColor)
              : null,
        );
      }).toList(),
    );
